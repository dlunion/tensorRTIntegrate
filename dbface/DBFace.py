...
class HSigmoidImpl(torch.autograd.Function):
    @staticmethod
    def symbolic(g, input):
        return g.op("Plugin", input, name_s="HSigmoid")

    def forward(self, x):
        out = F.relu6(x + 3, inplace=True) / 6
        return out

class HSwishImpl(torch.autograd.Function):
    @staticmethod
    def symbolic(g, input):
        return g.op("Plugin", input, name_s="HSwish")
...
class DBFace(nn.Module):
    ...
    def forward(self, x):
        ...
        tlrb = torch.exp(tlrb)
        pool_hm = F.max_pool2d(sigmoid_hm, kernel_size=3, padding=1, stride=1)
        return sigmoid_hm, tlrb, landmark, pool_hm
...
torch.onnx.export(model, dummy_input, "dbface.onnx", output_names=["hm", "tlrb", "landmark", "pool_hm"], opset_version=9)


'''
import torch.onnx.symbolic_opset9

# symbolic_opset9.py   line 743, changed
