...
#ifndef ChannelMultiplicationLayer_HPP
#define ChannelMultiplicationLayer_HPP

#include <plugin/plugin.hpp>

namespace Plugin {
	class ChannelMultiplicationLayer : public TRTPlugin {
	public:
		//设置插件函数，通过宏，执行插件创建函数，同时执行模式匹配名字，用来对每个层的名字做模式匹配
		//该匹配方法用的是ccutil::patternMatch，请参照这个函数
		SETUP_PLUGIN(ChannelMultiplicationLayer, "channel_multiplication*");
...
#endif //ChannelMultiplicationLayer_HPP