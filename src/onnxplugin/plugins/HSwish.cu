...
__global__ void HSwishKernel(_T* input, _T* output, _T bias, int edge);


template<>
__global__ void HSwishKernel(float* input, float* output, float bias, int edge) {

    KERNEL_POSITION;
    float x = input[position];
    float a = x + 3;
    a = a < 0 ? 0 : (a >= 6 ? 6 : a);
	output[position] = x * a / 6 + bias;
}

template<>
__global__ void HSwishKernel(halfloat* input, halfloat* output, halfloat bias, int edge) {
...
int HSwish::enqueue(const std::vector<GTensor>& inputs, std::vector<GTensor>& outputs, const std::vector<GTensor>& weights, void* workspace, cudaStream_t stream) {
	...
	if (config_->configDataType_ == TRTInfer::DataType::dtFloat) {
		HSwishKernel <<<grid, block >>> (inputs[0].ptr<float>(), outputs[0].ptr<float>(), bias, count);
	}
	else if (config_->configDataType_ == TRTInfer::DataType::dtHalfloat) {
		HSwishKernel <<<grid, block>>> (inputs[0].ptr<halfloat>(), outputs[0].ptr<halfloat>(), halfloat(bias), count);
	}
	return 0;
}
